...
<div class="wrap">
  ...
  <div class="body">
    ...
    <div class="msg">
      ...
      Your assets and download links are ready in your dedicated Google Drive directories. Click below to start importing into Premiere Pro, After Effects, DaVinci Resolve, or Final Cut.
    </div>
    <div class="cabinet">
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (razorpay_payment_id || razorpay_order_id || razorpay_signature) {
    ...
    console.log(`[Verify] Secret length: ${keySecret.length}, Order: ${razorpay_order_id}`);
    ...
    if (generated_signature !== razorpay_signature) {
      console.warn(`[Verify] MISMATCH — Generated: ${generated_signature}`);
      console.warn(`[Verify] MISMATCH — Received:  ${razorpay_signature}`);
      return res.status(400).json({ success: false, error: "Signature mismatch." });
    }

    console.info(`[Verify] Signature OK for order: ${razorpay_order_id}`);

    const secureToken = createSecureToken({ name, email, paid: true, method: "razorpay", timestamp: Date.now() });
